...
VIDEO_PATH = 'Bridge.avi'          # Your video path
...
def forward_splat(img, flow, factor):
    """
    Forward splatting using positive flow:
        (x, y) -> (x + factor*u, y + factor*v)

    Bilinear weights are used to distribute one source pixel
    to four neighboring target pixels.
    """
    h, w = img.shape[:2]
    ...
    target_x = grid_x + factor * flow[..., 0]
    target_y = grid_y + factor * flow[..., 1]
    ...
    def splat(xx, yy, ww):
        ...
        if not np.any(valid):
            return
        np.add.at(out, (yy[valid], xx[valid]), img_f[valid] * ww[valid, None])
        np.add.at(weight, (yy[valid], xx[valid]), ww[valid])
    ...
    # Normalize where splatted
    valid_w = weight > 1e-4
    out[valid_w] /= weight[valid_w, None]

    # Hole filling with OpenCV inpainting
    hole_mask = (~valid_w).astype(np.uint8) * 255
    out_u8 = np.clip(out, 0, 255).astype(np.uint8)

    if np.any(hole_mask):
        out_u8 = cv2.inpaint(out_u8, hole_mask, 3, cv2.INPAINT_TELEA)
...
def denoise_flow_dis(flow):
    u = cv2.medianBlur(flow[..., 0].astype(np.float32), 3)
    v = cv2.medianBlur(flow[..., 1].astype(np.float32), 3)
    ...
    thr = max(0.003, np.percentile(mag, 55) * 0.22)
    flow[mag < thr] = 0.0

    flow = np.clip(flow, -2.0, 2.0)
...
class MotionMagnifier:
    ...
    def process_video(self, video_path, start_frame=0, end_frame=None):
        ...
        for t in range(1, T):
            ...
            flow_t = flows_used[t]
            if self.name == "DIS":
                flow_t = denoise_flow_dis(flow_t)

                # Suppress top-sky artifacts
                flow_t[:40, :, :] = 0.0
            else:
                flow_t = cv2.GaussianBlur(flow_t, (5, 5), 1.0)

            mag_frame = forward_splat(first_frame, flow_t, MAG_FACTOR)
