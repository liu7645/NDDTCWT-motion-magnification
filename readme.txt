...
RAFT: For both estimation and magnification tasks, we utilize the official raft_large model provided by torchvision.models.optical_flow. The model is loaded with the default pre-trained weights (Raft_Large_Weights.DEFAULT).
...
Color Space: Input video frames are read using OpenCV, which stores frames in BGR format by default. For RAFT, the frames are explicitly converted from BGR to RGB before being transformed into tensors and passed to the model, because the torchvision RAFT implementation expects RGB image ordering. For DIS, the frames are converted from BGR to Grayscale (cv2.COLOR_BGR2GRAY), since the OpenCV DIS implementation in our baseline operates on grayscale images.
...
Inference Iterations: During displacement estimation, the RAFT model is run in evaluation mode with 10 flow updates. During motion magnification, the torchvision RAFT model uses its default inference behavior in evaluation mode.
...
Warping Strategy: For motion magnification rendering, the current implementation uses forward splatting with positive flow as the main synthesis strategy. Let I(x,y) be the reference frame and (u,v) be the extracted and temporally filtered optical flow at a given pixel. The target coordinates are calculated as: target_x = x + factor * u, target_y = y + factor * v.
Interpolation / Splatting: Each source pixel is distributed to neighboring target-grid locations using bilinear splatting weights.
Hole Handling: After forward splatting, uncovered pixels are identified from the accumulated splatting weights and converted into a binary hole mask. These holes are then filled using OpenCV inpainting (cv2.inpaint with the TELEA method).
Additional DIS-Specific Stabilization: For the DIS baseline, mild flow denoising is applied before rendering, and the top part of the frame is suppressed to reduce sky/background artifacts.
Limitation Note: Optical-flow-based motion magnification remains sensitive to flow-estimation errors and warping-related artifacts, especially under large amplification factors. As a result, geometric distortions, holes, tearing, or smoothing effects may still appear in the rendered outputs and should be considered when interpreting qualitative comparisons.
...
1. Read video frames with OpenCV (BGR).
2. Convert frames to RGB for RAFT or to grayscale for DIS.
3. Compute optical flow using RAFT or DIS.
4. Extract displacement signals.
5. Apply temporal bandpass filtering.
6. Use filtered flow for displacement estimation or motion magnification via forward splatting-based rendering with inpainting-based hole filling.
