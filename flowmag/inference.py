...
if __name__ == '__main__':
    ...
    sys.argv = [
        ...
        '--save_name', 'inference/result_Bridge',
        '--alpha', '40',  # 想改倍数？直接把这里的 20 改成你要的数字！
        '--output_video',
    ...
    mask_path = 'data/my_mask_for_bridge.png'
    if os.path.exists(mask_path):
         print(f"成功找到遮罩文件: {mask_path}，正在开启局部放大模式！")
         # 1. 以灰度图模式读取你做好的 mask
         mask_img = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
            # 2. 把 0-255 的像素值转换成 0.0 到 1.0 之间的张量 (Tensor)
         mask_tensor = torch.tensor(mask_img / 255.0, dtype=torch.float32)
            # 3. 如果电脑有 GPU，把 mask 也传进显存里，跟模型作伴
         if torch.cuda.is_available():
            mask_tensor = mask_tensor.cuda()

    else:
        print(f"警告：找不到 {mask_path}，将使用全图默认放大！")
        mask_tensor = None
        # ========================================================
    ...
    from pathlib import Path
        ...
        # 用 Path() 把纯文本包装成路径对象，完美塞进函数！
    inference(model, frames_dataset, Path(args.save_name), alpha=args.alpha, mask=mask_tensor, output_video=True)
