function Bridgedisplacement(dataDir, outDir)
    ...
    for mode =  1:1
        ...
        video_time = (timeOffset+1/samplingRate):1/samplingRate:(timeOffset + size(motion,4)/samplingRate);
            ...
            video_error_sd = mmPerPixel * sqrt(motion_covariance(pt(1), pt(2), 2, 2) * ...
                1);
            ...
            PlotData(video_displacement, video_time , accel_displacement, accelerometerTimestamps, video_error_sd, ylims{mode});
        % --- 视频位移数据 (CSV) ---
        video_table = table(video_displacement(:), video_time(:), 'VariableNames', {'pyr', 'videotime'});
        video_csv_name = fullfile(outDir, sprintf('pyr_displacement.csv'));
...
function PlotData(video_data, video_time, accel_data, accel_time, error_sd, ylims)
