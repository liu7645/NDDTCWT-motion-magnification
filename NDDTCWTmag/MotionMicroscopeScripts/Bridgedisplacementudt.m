function Bridgedisplacementudt(dataDir, outDir)
  video_file = 'Bridge.avi';
    ...
    for mode =  1:1
        ...
        timeOffset =  88.60 + 13700/samplingRate;
...
%         video_time = (timeOffset+3/samplingRate):1/samplingRate:(timeOffset +2/samplingRate+ size(motion,4)/samplingRate);
            ...
            video_displacement = squeeze(motion(pt(1), pt(2), 2, :));
            ...
            accel_displacement = IntegrateAcceleration(acceleration{1}, ...
            accelerometerTimestamps, accelerationHighpassCutoff, ...
            [loCutoff(mode), hiCutoff(mode)]);     


            % --- 视频位移数据 (CSV) ---
        video_table = table(video_displacement(:), video_time(:), 'VariableNames', {'pyr', 'videotime'});
        video_csv_name = fullfile(outDir, sprintf('udt_displacement.csv'));
        writetable(video_table, video_csv_name);
            accel_table = table(accelerometerTimestamps(:), accel_displacement(:), ...
            'VariableNames', {'time', 'acceleration'});
        accel_csv_name = fullfile(outDir, sprintf('accel_displacement.csv'));
        writetable(accel_table, accel_csv_name);
            
            
            PlotData(video_displacement, video_time , accel_displacement, accelerometerTimestamps, ylims{1});
                 
     
    
    end
end
function PlotData(video_data, video_time, accel_data, accel_time,  ylims)%error_sd,
