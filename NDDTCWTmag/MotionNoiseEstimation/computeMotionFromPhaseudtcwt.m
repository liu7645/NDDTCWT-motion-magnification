...
% Date: August 2016
function motion = computeMotionFromPhaseudtcwt(phaseDifferences, filterResponse, sigma, modulation, tuningFreq, useMex)
    if (nargin < 6)
        useMex = true;
    end
...
%     else
        ...
        weightSq = abs(filterResponse).^2 + eps; %W
        for c = 1:size(tuningFreq,1) %滤波器数量，层数及方向
        ...
        grady = Differentiate(lowpass, derivPrefilter, derivKernel);%卷积
...
function input = Differentiate(input, kernel_x, kernel_y)
    input = padarray(input, [2, 2], 'replicate', 'both');%边界复制填充扩大一倍
    kernel_x = kernel_x(:)';
    kernel_y = kernel_y(:);
