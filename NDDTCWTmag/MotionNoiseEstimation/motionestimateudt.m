function motionestimateudt(dataDir, outDir)
  video_file = 'Bridge.avi';
    ...
    % low-frequency noise
    accelerationHighpassCutoff = 0.2; % Hz
...
%     %video_file = 'Bridge.avi';
%     vr = VideoReader(fullfile(dataDir, video_file));
%     csvre = readmatrix(fullfile(dataDir, csvfile));
% %% 读取csv数据
% 
% %     acceleration{1} = csvre(51:551, 3)';
% %     acceleration{2} = csvre(51:551, 3)';
%     acceleration{1} = csvre(6200:7000, 5)';
%     acceleration{2} = csvre(6200:7000, 5)';
% 
%     accelerometerTimestamps = csvre(6200:7000, 1)';    % Modal parameters
%        loCutoff =[0.12 0.44]%[ 0.19 0.54] %[0.3 0.5 0.3]%[0.1,2.7]%[1.6, 0.1] %[1.6, 2.4];
%      hiCutoff = [0.38 0.72]%[0.26 0.61]
% 
% %     loCutoff =[ 0.19 0.54] %[0.3 0.5 0.3]%[0.1,2.7]%[1.6, 0.1] %[1.6, 2.4];
% %     hiCutoff = [0.26 0.61]%[0.5 0.7 0.7]%[5, 5]%[2.7, 1.6] %[1.8, 2.7];
%     sigma = 0;
%     samplingRate = 30;
%     temporalFilter = @Butterworth2ndOrder;
%      frameRange = [8000 8600];
%     % Points to plot the motion in (y, x) format
%     % First two are coincident with the accelerometers, last one is not
%     pt = [214 887];
%    % 205 62 ];
...
%    % ylims{2} = [-5 5];
% 
%     mmPerPixel = -65/34;   %-660/50*1.55%-24/42*25.4;48/40
    for mode =  1:2
...
%         filename{mode} = RunOnceOnly(motion_amp_filename, MotionAmplificationFunction);
        ...
        MotionComputationFunction = @(x) computeMotionAndCovarianceudtcwt(vr, ...
             'sigma', sigma,'temporalFilter', temporalFilterMotion, ...
              'useFrames', frameRange);%
...
%         video_time = (timeOffset+3/samplingRate):1/samplingRate:(timeOffset +2/samplingRate+ size(motion,4)/samplingRate);
        ...
        for ptIDX = 1:2
            ...
            accel_displacement = IntegrateAccelerationonline(acceleration{ptIDX}, ...
            ...
            PlotData(video_displacement, video_time , accel_displacement, accelerometerTimestamps, ylims{1}, outFilename);
            saveas(gcf,['bridgedis/' num2str(loCutoff(mode)) 'udt' num2str(ptIDX) '.jpg']);
            saveas(gcf,['bridgedis/' num2str(loCutoff(mode)) 'udt' num2str(ptIDX) '.fig']);            
    end    
    
    end
end
function PlotData(video_data, video_time, accel_data, accel_time,  ylims, filename)%error_sd,
   ff = figure('Color', [1 1 1], 'Position', [1 1 800 800], 'PaperSize', [8 8],  'PaperUnits', 'inches', 'PaperPosition', [0 0 8 8]);
...
%    plot([0, 1700000], -[1,1]*error_sd, 'm', 'LineWidth', 1);
   ...
   set(gca, 'Xtick', 545+[0 10 20 30 40 50]);
...
%     displacement = cumtrapz(accelTimestamps, velocity);
    displacementBandpassed = filter(B_band, A_band, accelerationHighpass); % m
    
    
end
function displacementBandpassed = IntegrateAccelerationonline(acceleration, accelTimestamps, highpassCutoff, bandpassCutoffs)
    accelerometerSamplingRate = 1./(accelTimestamps(2)-accelTimestamps(1));
    accelerometerNyquistRate = accelerometerSamplingRate / 2;
    ...
    %displacementB = filter(B_band, A_band, accelerationHighpass);
